...
- [x] Brush Workspace `Carpet` stage 3: the right side now includes a dedicated seamless preview that renders the carpet as one continuous 3x3 composition, highlights the active slot, uses the active selected variant in preview when available, keeps empty slots visible as future placements, and turns the carpet flow into a real spatial authoring pass instead of a plain list of variants
