...
- [x] Brush Workspace `Carpet` stage 3: the right side now includes a dedicated `Carpet Preview` that renders the full 3x3 composed layout, highlights the active slot, uses the active selected variant in preview when available, keeps empty slots visible as future placements, and turns the carpet flow into a real spatial authoring pass instead of a plain list of variants
