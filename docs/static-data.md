...
- existing replaced files are copied to `bkps` before the new file is written.
...
The exporter scans floors `0..7` and emits minimap and satellite chunks for each floor that contains visible tile data.
...
4. render satellite chunks from item sprites blended over minimap colors.
...
6. map chunks are rendered from `MapData.mapassets` using `topleft`, `widthsquare`, `heightsquare`, and `scale`.
...
8. validate changes on a real CIP-compatible client after exporter modifications.
