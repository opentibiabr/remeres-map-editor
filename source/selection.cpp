...
void Selection::add(const Tile* tile, std::shared_ptr<Item> item) {
	ASSERT(subsession);
	ASSERT(tile);
	ASSERT(item);
	...
	subsession->addChange(std::make_shared<Change>(new_tile));
}
...
void Selection::remove(Tile* tile, std::shared_ptr<Item> item) {
	ASSERT(subsession);
	ASSERT(tile);
	ASSERT(item);
